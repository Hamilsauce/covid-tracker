...
let fs = require('fs');
let fileContent = fs.readFileSync('c:\\Users\\jacob\\AppData\\Roaming\\Code\\User\\globalStorage\\alefragnani.project-manager\\projects.json')
const parseAndSort = data => {
  // let parsedData = JSON.parse(data);
  let projectString = JSON.stringify(data, null, 2);
  writeJson(projectString);
}
const writeJson = data => {
  fs.writeFileSync('./jsonOut.json', data)
}
const gLatest = [{
    "SWE": {
      "confirmed": 20302,
...
const flattenCountryData = (data) => { //! step one in getting top 10
  ...
  data.forEach(country => {
    let sumPerCountry = Object.entries(data[0])
      .map((array) => {
...
console.log(flattenCountryData(gLatest))
